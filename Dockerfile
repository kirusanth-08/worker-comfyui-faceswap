# Base image for RTX 5090 with CUDA 12.8 and cuDNN runtime
...
# Set environment for PyTorch and ComfyUI
ENV ENABLE_PYTORCH_UPGRADE=true
ENV PYTORCH_INDEX_URL=[https://download.pytorch.org/whl/cu128](https://download.pytorch.org/whl/cu128)
ENV CUDA_VERSION_FOR_COMFY=12.8
ENV COMFYUI_VERSION=latest

# Install Python, Git, wget, ffmpeg, and other dependencies
RUN apt-get update && apt-get install -y python3.12 python3.12-venv git wget libgl1 libglib2.0-0 libsm6 libxext6 libxrender1 ffmpeg && ln -sf /usr/bin/python3.12 /usr/bin/python && ln -sf /usr/bin/pip3 /usr/bin/pip && apt-get autoremove -y && apt-get clean -y && rm -rf /var/lib/apt/lists/*

# Install uv and create isolated virtual environment
RUN wget -qO- [https://astral.sh/uv/install.sh](https://astral.sh/uv/install.sh) | sh && ln -s /root/.local/bin/uv /usr/local/bin/uv && ln -s /root/.local/bin/uvx /usr/local/bin/uvx && uv venv /opt/venv
...
# Upgrade PyTorch to match CUDA 12.8 for RTX 5090
...
# Prevent pip from asking for input during uninstall steps in custom nodes
ENV PIP_NO_INPUT=1
...
# Stage 2: Downloader (for downloading models separately)
